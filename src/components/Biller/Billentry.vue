...
data(){
  return{
     medname:'',   quantity:'',  date:'dd/mm/yyyy',  Quantity:'',  billno:'1000', total:0,  gst:0,
  ...
  if(cn.MedicineName==this.medname)
 {
   if(Number(cn.quantity)>=this.quantity && this.quantity>=0){
    cn.quantity=Number(cn.quantity)-this.quantity;
     this.upquantity=Number(cn.quantity);
        if(cn.quantity>=0){
          this.snackbar=true;
...
if(i.MedicineName==this.medname){
  console.log("stpush"+this.upquantity)
this.stocktemp.push({MedicineName:i.MedicineName, quantity:Number(this.upquantity), unitprice:i.unitprice});
}
}
},
updated(med){        
...
for(let a of this.$store.state.Stock){
   for (let i of this.temp) {
     ...
     this.$store.state.billDetails.push({Billno:this.billno, MedicineName:i.MedicineName, quantity:i.quantity, unitprice:this.unitdetails, amount:this.amountdetails})
      console.log("details"+this.billno+ i.MedicineName+i.quantity);
      }
   }  
}
}
